<x-app-layout>
                    ...
                    @method('put')
                        <div class="-m-2">
                            ...
                            <div class="p-2 w-1/2 mx-auto">
                                ...
                                <textarea id="infomation" name="infomation" required rows="10" class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">{{ $product->infomation }}</textarea>
                                </div>
                            </div>
                            <div class="p-2 w-1/2 mx-auto">
                                <div class="relative">
                            ...
                            <div class="p-2 w-1/2 mx-auto">
                                ...
                                <input type="hidden" id="current_quantity" name="current_quantity" value="{{ $quantity }}" required class="text-base outline-none text-gray-700 py-1 px-3 leading-8">
                                <div class="w-full bg-gray-100 bg-opacity-50 rounded">{{ $quantity }}</div>
                                </div>
                            </div>
                            <div class="p-2 w-1/2 mx-auto">
                                <div class="relative flex justify-around">
                                    <div><input type="radio" name="type" value="{{ \Constant::PRODUCT_LIST['add'] }}" checked class="mr-2">追加</div>
                            ...
                            <div class="p-2 w-1/2 mx-auto">
                                <div class="relative flex justify-around">
                                    ...
                                    <input type="radio" name="is_selling" value="0" @if($product->is_selling === 0) { checked } @endif>停止中
                                </div>
                            </div>
                    ...
                    <form id="delete_{{ $product->id }}" action="{{ route('owner.products.destroy', ['product' => $product->id]) }}" method="post">
                        ...
                        @csrf
                            <div class="p-2 w-full mt-32 flex justify-around">
                                <a href="#" data-id="{{ $product->id }}" onclick="deletePost(this)" type="submit" class="text-white bg-red-400 border-0 py-2 px-4 focus:outline-none hover:bg-red-600 rounded">削除する</a>
                            </div>
                    </form>
                </div>
            </div>
    ...
    <script>
        ...
        // imageクラスが付いているもの全てを取得する

        images.forEach( image => {//1つずつ繰り返す
            image.addEventListener('click', function(e) {//クリックしたら
                // const imageName = e.target.dataset.id.substr(0, 6)//data-idの６文字
                const imageName = e.target.dataset.id.substr(0, 6)//data-idの６文字
                const imageId = e.target.dataset.id.replace(imageName + '_', '')//６文字カット=>idだけ取得
                //htmlでdata-〇〇と指定したものをe.target.dataset.〇〇で取得できる
                const imageFile = e.target.dataset.file
                const imagePath = e.target.dataset.path
                const modal = e.target.dataset.modal

                //サムネイルとiuput type=hiddenのvalueに設定
